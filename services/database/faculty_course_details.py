...
def get_faculty_course_details(db: Session, current_faculty: Dict[str, Any], assigned_course_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get ONLY students with formal enrollment records in assigned_course_approval
        ...
        # Debug: Show details of each student found
        for i, (student, user, approval) in enumerate(students_query):
            print(f"Student {i+1}: ID={student.id}, Name={user.first_name} {user.last_name}, Status={approval.status}")
        ...
        # Get recent attendance records (optimized query)
        ...
        return {
            ...
            "attending_students": [],  # Always empty now since we only show formal enrollments
